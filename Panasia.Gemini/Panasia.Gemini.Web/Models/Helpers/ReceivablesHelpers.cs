...
{
    public class ReceivablesHelpers
    {
        ...
        public bool ImportFromExcel(string filePath)
        {
            ...
            try
            {
                ...
                dtRead.Columns.Add("RecNum").SetOrdinal(0);//首列插入系列号
                //DataTable dtMerge = new DataTable();//存放子表信息
                for (int i = 0; i < dtRead.Rows.Count; i++)
                {
                    string sheetCoding = "R" + DateTime.Now.ToString("yyyyMMddHHmmss") + (i+1).ToString("000");//生成系列号
                    dtRead.Rows[i][0] =  sheetCoding;//给主表、子表添加系列号信息
                }
         ...
         string[] DtToStrings(DataTable dt)
        {
            ...
            {
                //判断条例是否已经存在 
                //string strsel = string.Format("select * from acc_Receivables where RecNum='{0}' and OccurencyDate='{1}'  and CompanyID='{2}' and BankName='{3}' and BankAccount='{4}' and PaymentName='{5}'and PaymentMoney='{6}' and Remark='{7}'", dt.Rows[i][1].ToString(), s);
                ...
                strSql[i] = "insert into acc_Receivables values(" + str + ",0,1,'" + UserID + "','" + dtime + "','" + UserID + "','" + dtime + "')";
            }
            return strSql;
        }
         //添加导入信息
