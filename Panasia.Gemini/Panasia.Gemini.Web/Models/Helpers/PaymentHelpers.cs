...
namespace Panasia.Gemini.Web.Models.Helpers
{
    public class PaymentHelpers
    {
        ...
        public ResultData ImportResult(string filePath,string type)
        {
            ImportFromExcel(filePath,type);
            return result;
        }
        ...
        //导入
        public bool ImportFromExcel(string filePath,string type)
        {
            ...
            try
            {
                ...
                dtRead.Columns.Add("PaymentID").SetOrdinal(0);//首列插入系列号
                //DataTable dtMerge = new DataTable();//存放子表信息
                for (int i = 0; i < dtRead.Rows.Count; i++)
                {
                    string sheetCoding = "";//生成系列号
                    SysContext db = new SysContext();
                    sheetCoding = db.Database.SqlQuery<string>("DECLARE @Pay varchar(50); EXEC sp_CreateSerialCode 'Payment','P',6,@Pay output;select @Pay",
                        "Paymet").First();
                    dtRead.Rows[i][0] = sheetCoding.ToString();//给主表、子表添加系列号信息
                }
                dtRead = AddInfo(dtRead);//添加导入信息
                string[] strsInsert = DtToStrings(dtRead,type);//生成SQL执行语句
        ...
        public DataSet ReadExlToDt(string filePath)
        {
         ...
         //生成SQL执行语句
         string[] DtToStrings(DataTable dt,string type)
        {
            string[] strSql = new string[dt.Rows.Count];
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                //判断条例是否已经存在 
                //string strsel = string.Format("select * from bi_Payment where RecNum='{0}' and OccurencyDate='{1}'  and CompanyID='{2}' and BankName='{3}' and BankAccount='{4}' and PaymentName='{5}'and PaymentMoney='{6}' and Remark='{7}'", dt.Rows[i][1].ToString(), s);
                ...
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    ...
                    {
                        SysContext db = new SysContext();
                        string companyname= dt.Rows[i][j].ToString().Trim();
                        string companyid = db.bi_Payment.Single(b => b.PaymentName == companyname).PaymentID;
                        str += type+",'" + companyid.Replace("'", "\"") + "'";
                    }
                    else
                    {
                ...
                strSql[i] = "insert into bi_Payment values(" + str + ",1,'" + UserID + "','" + dtime + "','" + UserID + "','" + dtime + "')";
            }
            return strSql;
        }
         //添加导入信息
