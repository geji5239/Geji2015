...
{
    public class acc_ExpenditureHelpers
    {
        ...
        public ResultData ImportResult(string filePath)
        {
        ...
        //导入
        public bool ImportFromExcel(string filePath)
        {
            ...
            try
            {
                ...
                dtRead.Columns.Add("ExpenditureID").SetOrdinal(0);//首列插入系列号
                for (int i = 0; i < dtRead.Rows.Count; i++)
                {
                    string sheetCoding = "";//生成系列号
                    SysContext db = new SysContext();
                    sheetCoding = db.Database.SqlQuery<string>("DECLARE @Pay varchar(50); EXEC sp_CreateSerialCode 'Expenditure','E',6,@Pay output;select @Pay",
                        "Paymet").First();
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < 7; j++)
                {
                    ...
                    if (j == 2)
                    {
                        ...
                        string companyid = db.hr_Companies.Single(c => c.Name == companyname).CompanyID;
                        str += "'" + companyid.Replace("'", "\"") + "'";
                    }
                    else
                    {
                ...
                for (int j = 6; j < dt.Columns.Count; j++)
                {
                    ...
                    if (j == 6)
                    {
                        str1 += "'" + dt.Rows[i][0].ToString().Replace("'", "\"") + "'";
                    }
                    else
                    {
                        str1 += "'" + dt.Rows[i][j].ToString().Replace("'", "\"") + "'";
                    }
                    
                }
                string dtime = DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
                strSql[i] = "insert into acc_Expenditure values(" + str + ",1,'" + UserID + "','" + dtime + "','" + UserID + "','" + dtime + "');";
                strSql[i] += "insert into acc_ExpenditureDetail values(" + str1 + ",1,'" + UserID + "','" + dtime + "','" + UserID + "','" + dtime + "');";
            }
            return strSql;
        }
         //添加导入信息
